use getopts::Options;
use hext;
use hext::Options as HextOptions;
use std::env;
use std::fs;
use std::fs::File;
...
fn main() {
    ...
    opts.optflag("p", "pad-bits", "pad bits until they reach an octet");
    ...
    let allow_unaligned_bits = matches.opt_present("p");

    let hext_options = HextOptions {
        allow_unaligned_bits
    };
    ...
    if files.len() == 0 {
        ...
        do_hext(&raw, &mut outfile, &hext_options);
    } else {
        for file in files {
...
fn do_hext(raw: &str, outfile: &mut Option<File>, options: &HextOptions) {
    match hext::to_bytes(raw, options) {
